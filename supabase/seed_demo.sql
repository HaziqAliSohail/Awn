-- ============================================================
-- Awn — DEMO seed data (accounts + needs + reports).
-- Run in the Supabase SQL Editor for project "Awn"
-- (ynnjtahmjkblamtmezxj). Runs as the postgres role.
--
-- Everything created here is tagged so it can be removed in one
-- step later with seed_demo_teardown.sql:
--   * demo users     -> auth.users.id LIKE 'aaaaaaaa-%'
--   * demo profiles  -> same ids (FK to auth.users)
--   * demo needs      -> sprints.creator_id LIKE 'aaaaaaaa-%'
--   * demo reports   -> reports.reporter_id LIKE 'aaaaaaaa-%'
--
-- Safe to re-run: inserts are guarded with ON CONFLICT / NOT EXISTS.
-- All demo accounts share the password  AwnDemo123!  (login optional;
-- they exist mainly to populate names, needs, and the admin queue).
-- ============================================================

create extension if not exists pgcrypto;

-- ── 1) 30 demo accounts (auth.users) ────────────────────────
with names as (
...
select
  ...
  'demo'||n||'@awn-demo.local',
  crypt('AwnDemo123!', gen_salt('bf')),
  now(), now() - (n || ' days')::interval, now(),
...
-- ── 2) 30 profiles (FK to the accounts above) ───────────────
with names as (
  select * from unnest(array[
   'Ahmed Khan','Fatima Ali','Yusuf Rahman','Aisha Siddiqui','Bilal Ahmed',
   'Zainab Hussain','Omar Farooq','Maryam Iqbal','Ibrahim Malik','Khadija Noor',
...
select
  ...
  (array[
    array['meals_food','transport_errands']::domain_category[],
    array['home_repairs','moving_labor']::domain_category[],
    array['quran_islamic','tutoring_education']::domain_category[],
    array['elder_sick_care','health_guidance']::domain_category[],
    array['career_resume','tech_digital']::domain_category[]
  ])[1+(n%5)],
  '{}'::domain_category[],
...
-- ── 3) 45 needs (sprints) ───────────────────────────────────
-- 15 templates cycled 3x; creator assigned round-robin; a spread
-- of statuses; ~6 auto-flagged so the screening queue is populated.
with tpl as (
  select * from (values
    ...
    ( 2,'meals_food',        'Meals for a family after a new baby',         'Sister just had a baby, the family could really use a week of home cooked meals',        '["Set up a meal rota","Deliver dinners for five days"]',                     'this_week', true),
...
insert into public.sprints (
  ...
  timing, in_person, status, flagged, flag_reason, created_at
)
select
...
from grid
where not exists (
  -- keep re-runs from duplicating: skip if this creator already has this title
  select 1 from public.sprints s
  where s.title = grid.title
    and s.creator_id = ('aaaaaaaa-0000-4000-8000-'||lpad((1+(grid.n%30))::text,12,'0'))::uuid
);

-- ── 4) 14 open reports (populate the moderation queue) ───────
-- Odd rows report a need; even rows report a member. Reporter is
-- always a demo member; nothing here targets the real admin.
...
-- ── 5) suspend 2 demo members (populate the suspended list) ──
